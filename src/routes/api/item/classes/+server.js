...
export async function GET({ locals }) {
  ...
  const values = await models.ItemClass.findAll({
...
export async function POST({ request, locals }) {
  ...
  try {
    ...
    const created = await models.ItemClass.create({
      tenantId: locals.tenantId,
      name: body.name || '',
      displayOrder: body.displayOrder || 1,
      product: Boolean(body.product),
      inventoryManagement: Boolean(body.inventoryManagement)
    });
...
export async function PUT({ request, locals }) {
  ...
  try {
    const body = await request.json();
    const item = await models.ItemClass.findOne({
...
export async function DELETE({ request, locals }) {
  ...
  try {
    const body = await request.json();
    await models.ItemClass.destroy({
      where: { id: body.id, tenantId: locals.tenantId }
    });
