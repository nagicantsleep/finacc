...
export const get_details = async (fy, account, sub_account, tenantId) => {
  ...
  for (let mon = new Date(fy.startDate); mon <= endDate; mon.setMonth(mon.getMonth() + 1)) {
    let where;
    if (sub_account && sub_account > 0) {
      where = {
        [Op.and]: {
          [Op.or]: [
            { debitAccount: account, debitSubAccount: sub_account },
            { creditAccount: account, creditSubAccount: sub_account }
          ],
    ...
    const details = await models.CrossSlipDetail.findAll({
      ...
      include: [
        { model: models.CrossSlip, as: 'crossSlip', where: { tenantId }, required: false },
        {
          model: models.Voucher,
          required: false,
          as: 'debitVoucher',
          where: { tenantId },
          include: [{ model: models.VoucherFile, as: 'files', where: { tenantId }, required: false }]
        },
        {
          model: models.Voucher,
          required: false,
