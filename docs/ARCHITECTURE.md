...
SSR first-paint via `load`: journal month, ledger account, trial balance v2, company list (`/company`, `/company/home`, `/company/new`, `/company/entry/:id`), accounts chart (`/accounts`), voucher list/entry (`/voucher`, `/voucher/new`, `/voucher/entry/:id`).
